...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (confirmedOrder && confirmedOrder.items && confirmedOrder.items.length > 0) {
    ...
    tbody.innerHTML = ''; // Очистка на случай повторного вызова
    ...
    confirmedOrder.items.forEach(item => {
      ...
      row.innerHTML = `
        ...
        <td style="border: 1px solid #ccc; padding: 8px;">${item.unitPrice} դր.</td>
        <td style="border: 1px solid #ccc; padding: 8px;">${item.subtotal} դր.</td>
      `;
